...
class Visualization(HasTraits):
    ...
    seq_name = Str('0')
    label_name = Str('gt    standing    sit    left_h_hold    right_h_hold    drink    walk    bending    clapping    phone_call    pointing')
    gt = Str('')
    est = Str('')
    ...
    def __init__(self, data_set):
        ...
        self.net.load_state_dict(torch.load('./models/action-net-300-140.pkl', map_location=torch.device('cpu')))
        self.net.eval()
    ...
    def _next_seq_fired(self):
        ...
        self.trait_set(seq_num=seq_num, seq_name=self.data_set.get_name(seq_num), gt=lbl, est=out)
    ...
    def update_seq_num(self):
        ...
        self.scene.mlab.clf()
        self.plot = self.scene.mlab.points3d(x, y, z, s, colormap='hot', scale_factor=1, scale_mode='none')
        self.trait_set(seq_name=self.data_set.get_name(seq_num), gt=lbl, est=out)
        # self.anim(self)
